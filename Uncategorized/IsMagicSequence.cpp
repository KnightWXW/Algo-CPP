...
//      然后请小扣思考一个 N 张卡牌的排列 target。魔术师的目标是找到一个数字 k（k >= 1），
...
//          若当前卡牌数量大于 k，则取走前 k 张卡牌，剩余卡牌继续重复这两个步骤，
//          直至所有卡牌全部被取走；
...
bool IsMagicSequence(vector<int>& target);
...
int main()
{
    ...
    printVecElement(vec);
    bool ans_A = IsMagicSequence(vec);
    printf("是否存在这个数字 k 使得「魔术取数排列」恰好就是 target? \n");
    printBool(ans_A);
}
