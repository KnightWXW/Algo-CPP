...
//      你有一个电影租借公司和 n 个电影商店。你想要实现一个电影租借系统，
//      它支持查询、预订和返还电影的操作。同时系统还能生成一份当前被借出电影的报告。
//      所有电影用二维整数数组 entries 表示，
//      其中 entries[i] = [shopi, moviei, pricei] 表示商店 shopi 有一份电影 moviei 的拷贝，
...
//                  将结果用二维列表 res 返回，其中 res[j] = [shopj, moviej]
//                  表示第 j 便宜的已借出电影是从商店 shopj 借出的电影 moviej 。
//                  res 中的电影需要按 价格 升序排序；如果价格相同，则 shopj 较小 的排在前面；
//                  如果仍然相同，则 moviej 较小 的排在前面。如果当前借出的电影小于 5 部，
//                  则将它们全部返回。如果当前没有借出电影，则返回一个空的列表。
...
//      注意：测试数据保证 rent 操作中指定商店拥有 未借出 的指定电影，
//          且 drop 操作指定的商店 之前已借出 指定电影。
//      示例 1：
//      输入：
...
//                      商店 1，0 和 2 有未借出的 ID 为 1 的电影。
//                      商店 1 最便宜，商店 0 和 2 价格相同，所以按商店编号排序。
//          movieRentingSystem.rent(0, 1); // 从商店 0 借出电影 1 。
//                      现在商店 0 未借出电影编号为 [2,3] 。
//          movieRentingSystem.rent(1, 2); // 从商店 1 借出电影 2 。
//                      现在商店 1 未借出的电影编号为 [1] 。
//          movieRentingSystem.report();   // 返回 [[0, 1], [1, 2]] 。
//                      商店 0 借出的电影 1 最便宜，然后是商店 1 借出的电影 2 。
//          movieRentingSystem.drop(1, 2); // 在商店 1 返还电影 2 。
//                      现在商店 1 未借出的电影编号为 [1,2] 。
//          movieRentingSystem.search(2);  // 返回 [0, 1] 。
//                      商店 0 和 1 有未借出的 ID 为 2 的电影。商店 0 最便宜，然后是商店 1 。
//      提示：
//          1 <= n <= 3 * 105
...
{
    MovieRentingSystem *movieRentingSystem = new MovieRentingSystem(3, [ [ 0, 1, 5 ], [ 0, 2, 6 ], [ 0, 3, 7 ], [ 1, 1, 4 ], [ 1, 2, 7 ], [ 2, 1, 5 ] ]);
    vector<int> s1 = movieRentingSystem.search(1);
    printVecElement(s1);
    ...
    vector<vector<int>> r1 = movieRentingSystem.report();
    print2DVecElement(r1);
    // 返回 [[0, 1], [1, 2]] 。商店 0 借出的电影 1 最便宜，然后是商店 1 借出的电影 2 。
    movieRentingSystem.drop(1, 2);
    // 在商店 1 返还电影 2 。现在商店 1 未借出的电影编号为 [1,2] 。
    vector<int> s2 = movieRentingSystem.search(2);
    printVecElement(s2);
