...
//              平均时间会根据截至目前所有从 startStation 站 直接
//              到达 endStation 站的行程进行计算，
//              也就是从 startStation 站进入并从 endStation 离开的行程
//              从 startStation 到 endStation 的行程时间与
//              从 endStation 到 startStation 的行程时间可能不同
//              在调用 getAverageTime 之前，
//              至少有一名乘客从 startStation 站到达 endStation 站
//              你可以假设对 checkIn 和 checkOut 方法的所有调用都是符合逻辑的。
//              如果一名乘客在时间 t1 进站、时间 t2 出站，那么 t1 < t2 。
//              所有时间都按时间顺序发生。
//      示例 1：
//      输入
...
//      输出
//      [null,null,null,null,null,null,null,14.00000,11.00000,null,11.00000,
//      null,12.00000]
//      解释
...
int main()
{
    ...
    undergroundSystem->checkIn(5, "Leyton", 10);
    undergroundSystem->checkOut(5, "Paradise", 16);                      // 乘客 5 "Leyton" -> "Paradise" ，用时 16-10 = 6
    double g2 = undergroundSystem->getAverageTime("Leyton", "Paradise"); // 返回 5.50000 ，(5 + 6) / 2 = 5.5
    printf("平均时间为 %d \n", g2);
    undergroundSystem->checkIn(2, "Leyton", 21);
    undergroundSystem->checkOut(2, "Paradise", 30);                      // 乘客 2 "Leyton" -> "Paradise" ，用时 30-21 = 9
    double g3 = undergroundSystem->getAverageTime("Leyton", "Paradise"); // 返回 6.66667 ，(5 + 6 + 9) / 3 = 6.66667
    printf("平均时间为 %d \n", g3);
}